...
def plot_training_progress(progress_df, target_name, scale_tag, run_timestamp):
    """
...
def train_model(df, features, target_name, scale_tag='all', run_timestamp='unknown'):
    ...
    stage_estimators = list(range(50, 501, 50))
    total_stages = len(stage_estimators)

    model = RandomForestRegressor(
        n_estimators=stage_estimators[0],
        max_depth=18,
        min_samples_leaf=5,
        max_features='sqrt',
        n_jobs=-1,
...
if __name__ == "__main__":
    ...
    for scale in training_scales:
        ...
        rent_model = train_model(df, feature_cols, 'rent', scale_tag, run_timestamp)
        return_model = train_model(df, feature_cols, 'return', scale_tag, run_timestamp)
