...
from Optimize_PLA_Delta import optimize_evrp_with_pla_delta
...
def run_optimization_pipeline(
    ...
    # -----------------------------------------------------------------
    if verbose:
        print(f"[{experiment_id}] Delta-PLA Pipeline | P={P_intervals} | Geo-Fence={'✓' if geo_fencing else '✗'}")
        print("=" * 60)
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("Step 5/6: 执行 Gurobi PLA-MIP 求解 (增量 Delta 模型)")
        print("=" * 60)

    progress = GurobiProgressTracker(label=experiment_id) if verbose else None

    t_start = time.perf_counter()
    model = optimize_evrp_with_pla_delta(
        grids=active_grids,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Delta-PLA 优化引擎 —— E-Bike Battery Swapping VRP",
...
使用示例:
  ...
  python main_delta.py --data path/to/other.csv                            # 指定数据文件
        """,
    )
    parser.add_argument(
    ...
    if args.random:
        ...
        print("=" * 60)
        print("  [随机] 随机选取模式 — Delta-PLA 优化引擎")
        if args.seed is not None:
            print(f"  随机种子: {args.seed}")
    elif args.datetime is not None:
        target_datetime = args.datetime
        print("=" * 60)
        print("  [指定] 用户指定日期时间 — Delta-PLA 优化引擎")
    else:
        target_datetime = DEFAULT_TARGET_DATETIME
        print("=" * 60)
        print("  [默认] 默认日期时间 — Delta-PLA 优化引擎")
